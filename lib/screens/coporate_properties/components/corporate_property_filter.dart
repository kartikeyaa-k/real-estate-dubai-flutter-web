...
class MyPropertyFilter extends StatefulWidget {
  const MyPropertyFilter({Key? key, this.isFullPageView = false, this.isInProcessPage = false}) : super(key: key);
...
enum PropertyStatus { InViewing, Negotiating, ContractApproval }
...
class _MyPropertyFilterState extends State<MyPropertyFilter> {
  ...
  final List<_Status> _propertyStatusCheckList = PropertyStatus.values.map((e) {
    ...
    return _Status(text: name.replaceAllMapped(RegExp(r'([A-Z])'), (m) => ' ${m[1]}'));
  }).toList();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Search bar
          Text("Search", style: TextStyles.body14.copyWith(color: kLightBlue)),
          SizedBox(height: 4),
          PrimaryTextField(controller: _searchController, text: "City, Tower Or Community"),
          SizedBox(height: Insets.xl),

          // Divider
          Row(
            children: [Expanded(child: Divider(endIndent: 15)), Text("OR"), Expanded(child: Divider(indent: 15))],
          ),
          SizedBox(height: Insets.xl),

          // Other Filters
          Text("Property Type", style: TextStyles.body14.copyWith(color: kLightBlue)),
          SizedBox(height: 4),
          PrimaryDropdownButton(/*value: "dummy",*/ itemList: [DropdownMenuItem(child: Text("data"))]),
          SizedBox(height: Insets.xl),

          Text("Pay", style: TextStyles.body14.copyWith(color: kLightBlue)),
          SizedBox(height: 4),
          PrimaryDropdownButton(/*value: "dummy",*/ itemList: [DropdownMenuItem(child: Text("data"))]),
          SizedBox(height: Insets.xl),

          Text("Agency", style: TextStyles.body14.copyWith(color: kLightBlue)),
          SizedBox(height: 4),
          PrimaryDropdownButton(/*value: "dummy",*/ itemList: [DropdownMenuItem(child: Text("data"))]),
          SizedBox(height: Insets.xl),

          if (widget.isInProcessPage) ...[
            Text("Select Status", style: TextStyles.body14.copyWith(color: kLightBlue)),
