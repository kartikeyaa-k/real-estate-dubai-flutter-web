...
class MyRequestedServicesTabView extends StatefulWidget {
  MyRequestedServicesTabView({Key? key}) : super(key: key);

  @override
  _MyRequestedServicesTabViewState createState() => _MyRequestedServicesTabViewState();
}

class _MyRequestedServicesTabViewState extends State<MyRequestedServicesTabView> {
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          BlocBuilder<RequestedServiceCubit, RequestedServiceState>(
            buildWhen: (previous, current) => previous.requestedServices != current.requestedServices,
            builder: (context, state) {
              final requestedService = state.requestedServices;

              if (state.status.isSubmissionSuccess) {
                if (requestedService.isEmpty)
                  return Container(
                    height: MediaQuery.of(context).size.height - kToolbarHeight - Insets.footerSize,
                    child: Center(child: Text("No Requested Service Found")),
                  );
                ...
                return Padding(
                  ...
                  child: GridView.builder(
                    ...
                    gridDelegate: SliverGridDelegateWithMaxCrossAxisExtentAndMinHeight(
                      ...
                      crossAxisSpacing: 30,
                      mainAxisSpacing: 30,
                      childAspectRatio: Responsive.isMobile(context) ? 328 / 243 : childAspectRatio,
                    ),
                    itemCount: requestedService.length,
